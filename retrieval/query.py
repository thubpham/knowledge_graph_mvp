...
def query(kg: KnowledgeGarden, question: str, client: LLMClient, now: datetime):
    ...
    if not kg.node_exists(anchor_id):
        return {
    ...
    if query_intent.pattern == "path":
        ...
        if not kg.node_exists(query_intent.target_entity):
            return {
                "error": "target entity not found",
                "target_entity": query_intent.target_entity
            }

    anchor_node = kg.get_node(anchor_id)
    returned_edges = []
    ...
    if query_intent.pattern == "direct_lookup":
        ...
        target_node = kg.get_node(query_intent.target_entity)
